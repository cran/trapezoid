...
\section{Examples}
\subsection{Simple trapezoid}
The generalized trapezoidal distribution simplifies to the simple trapezoid distribution when \begin{math} m = n = 2 \text{ and } \alpha = 1\end{math}. 

<<simpleTrapezoid, echo=TRUE, results=verbatim, fig=TRUE, include=FALSE>>=
...
density <- dtrapezoid(x, min = 0, mode1 = 1/3, mode2 = 2/3, max = 1,
	n1 = 2, n3 = 2, alpha = 1)
simpleTrapezoid <- ggplot(data = data.frame(x, density), 
		aes(x = x, y = density)) + geom_line() + theme_bw()
print(simpleTrapezoid)
@
\begin{figure}[H]
\centering
\caption{A simple trapezoidal distribution.}
\includegraphics{trapezoid-simpleTrapezoid}
\end{figure}



\subsection{Simple triangle}
The simple trapezoidal distribution further simplifies to the simple triangular distribution when \begin{math} b = c\end{math}. 

<<simpleTriangle, echo=TRUE, results=verbatim, fig=TRUE, include=FALSE>>=
# Simple triangle
x <- seq(from = 0, to = 1, by = 0.01)	
density <- dtrapezoid(x, min = 0, mode1 = 1/2, mode2 = 1/2, max = 1,
	n1 = 2, n3 = 2, alpha = 1)
simpleTriangle <- ggplot(data = data.frame(x, density), 
				aes(x = x, y = density)) + geom_line() + theme_bw()
print(simpleTriangle)
@
\begin{figure}[H]
\centering
\caption{A simple triangular distribution.}
\includegraphics{trapezoid-simpleTriangle}
\end{figure}
...
<<generalizedTrapezoids, echo=TRUE, results=verbatim, fig=TRUE, include=FALSE>>=
